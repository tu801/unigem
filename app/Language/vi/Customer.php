<?php
return [
    // register
    'fullName'                                   => 'Họ tên',
    'phoneNumber'                                => 'Số điện thoại',
    'email'                                      => 'Email',
    'password'                                   => 'Mật khẩu',
    'passwordConfirm'                            => 'Xác nhận mật khẩu',
    'address'                                    => 'Địa chỉ',
    'select_country'                             => 'Chọn quốc gia',
    'vietnam_country'                            => 'Việt Nam',
    'other_country'                              => 'Quốc gia khác',

    'activate_account'                           => 'Kích hoạt tài khoản',

    // customer profile
    'my_account'                                 => 'Tài khoản của tôi',
    'welcome_back'                               => 'Chào bạn, {0}!',
    'dashboard_message'                          => 'Tại đây bạn có thể quản lý thông tin tài khoản của mình, xem lịch sử đơn hàng và cập nhật địa chỉ giao hàng.',
    'logout'                                     => 'Đăng xuất',
    'dashboard'                                  => 'Bảng điều khiển',
    'account_detail'                             => 'Thông tin tài khoản',
    'change_password'                            => 'Đổi mật khẩu',
    'current_password'                           => 'Mật khẩu hiện tại',
    'old_password_not_match'                     => 'Mật khẩu cũ không đúng',
    'change_password_success'                    => 'Đổi mật khẩu thành công! Vui lòng đăng nhập với mật khẩu mới của bạn.',
    
    'orders'                                     => 'Đơn hàng',
    'editCustomerProfileLog'                     => '#{0} dã chỉnh sửa thông tin tài khoản',
    'editProfileSuccess'                         => 'Cập nhật thông tin tài khoản thành công',

    'shipping_address'                           => 'Địa chỉ giao hàng',
    'add_new_address'                            => 'Thêm địa chỉ mới',
    'ship_telephone'                             => 'Số điện thoại người nhận',
    'set_default_address'                        => 'Đặt làm địa chỉ mặc định',
    'default_address'                            => 'Địa chỉ mặc định',
    'ship_full_name_required'                    => 'Vui lòng nhập họ tên người nhận',
    'ship_telephone_required'                    => 'Vui lòng nhập số điện thoại người nhận',
    'ship_address_required'                      => 'Vui lòng nhập địa chỉ nhận hàng',
    'ship_telephone_unique'                      => 'Số điện thoại người nhận đã tồn tại',
    'address_added_successfully'                 => 'Địa chỉ giao hàng đã được thêm thành công',
    'edit_shipping_address'                      => 'Chỉnh sửa địa chỉ giao hàng',
    'delete_shipping_address'                    => 'Xóa địa chỉ giao hàng',
    'ship_address_not_found'                     => 'Địa chỉ giao hàng không tồn tại.',
    'address_updated_successfully'               => 'Địa chỉ giao hàng đã được cập nhật thành công',
    'address_deleted_successfully'               => 'Địa chỉ giao hàng đã được xóa thành công',
    'confirm_delete_address'                     => 'Bạn có chắc chắn muốn xóa địa chỉ này không? Hành động này không thể hoàn tác.',
    'deleteCustomerShippingAddressLog'           => '#{0} đã xóa địa chỉ giao hàng #{1}',

    // validation message
    'cus_full_name_required'                    => 'Vui lòng nhập họ tên',
    'cus_full_name_min_length'                  => 'Họ tên phải có ít nhất 3 ký tự',
    'cus_phone_required'                        => 'Vui lòng nhập số điện thoại',
    'cus_phone_invalid'                         => 'Số điện thoại không hợp lệ',
    'cus_phone_unique'                          => 'Số điện thoại đã tồn tại',
    'cus_phone_min_length'                      => 'Số điện thoại phải có ít nhất 8 ký tự',
    'cus_email_required'                        => 'Vui lòng nhập email',
    'cus_email_valid_email'                     => 'Email không đúng định dạng',
    'cus_email_is_unique'                       => 'Email đã tồn tại',
    'cus_address_required'                      => 'Vui lòng nhập địa chỉ',
    'password_required'                         => 'Vui lòng nhập mật khẩu',
    'password_min_length'                       => 'Mật khẩu phải có ít nhất 6 ký tự',
    'password_confirm_required'                 => 'Vui lòng nhập lại mật khẩu',
    'password_confirm_matches_password'         => 'Mật khẩu không trùng khớp',
    'country_required'                          => 'Vui lòng chọn quốc gia',
    'province_required'                         => 'Vui lòng chọn tỉnh/thành phố',
    'district_required'                         => 'Vui lòng chọn quận/huyện',
    'ward_required'                             => 'Vui lòng chọn phường/xã',
    'processing'                                => 'Đang xử lý...',
];
