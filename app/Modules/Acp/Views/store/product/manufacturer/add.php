            ...
            <div class="card card-outline card-primary">
                <div class="card-body pad">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="manufacture_name" class="form-control <?= session('errors.manufacture_name') ? 'is-invalid' : '' ?>" placeholder="<?= lang('ProductManufacturer.name') ?>" value="<?= old('manufacture_name') ?>">
                    </div>

                    <div class="form-group">
                        <label><?= lang('ProductManufacturer.description') ?> <span class="text-danger">*</span></label>
                        <textarea rows="3" name="description" class="form-control <?= session('errors.description') ? 'is-invalid' : '' ?>" placeholder="<?= lang('ProductManufacturer.description') ?>"><?= old('description') ?></textarea>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="custom-select form-control" name="status">
                            <?php foreach (ProductStatusEnum::toArray() as $item): ?>
                                <option value="<?= $item ?>" <?= old('status') == $item ? 'selected' : ''  ?> ><?= lang("ProductManufacturer.status_{$item}") ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
