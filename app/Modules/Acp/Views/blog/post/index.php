            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">
                        <a class="<?= ($listtype == 'all') ? 'badge badge-primary text-light' : 'text-primary' ?>"
                            href="<?= base_url("acp/post?listtype=all") ?>"><?= lang('Post.list_all_post') ?></a> |
                        <a class="<?= ($listtype == 'user') ? 'badge badge-primary text-light' : 'text-primary' ?>"
                            href="<?= base_url("acp/post?listtype=user") ?>"><?= lang('Post.list_user_post') ?></a> |
                        <a class="<?= ($listtype == 'deleted') ? 'badge badge-primary text-light' : 'text-primary' ?>"
                            href="<?= base_url("acp/post?listtype=deleted") ?>"><?= lang('Post.list_delete_post') ?></a>
                    </div>

                    <div class="card-tools mt-2">
                        <div class="input-group input-group-sm">
                            <input name="listtype" id="listtype" value="<?= $listtype ?? 'user'; ?>"
                                class="form-control" type="hidden" />
                            <input type="text" value="<?= (isset($search_title)) ? $search_title : '' ?>" name="title"
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-8 mb-2">
                            <div class="btn-group">
                                <a href="<?= route_to('add_post') ?>" class="btn btn-normal btn-primary btn-sm"
                                    title="Add New Post">
                                    <i class="fa fa-plus text"></i> <?= lang('Post.title_add') ?>
                                </a>
                                <button type="submit" name="mdelete" class="btn btn-default btn-sm ml-2"><i
                                ...
                                <div class="col-md-5">
                                    ...
                                    <div id="chkCategory" class="form-group">
                                        <select class="custom-select form-control" name="category">
                                            ...
                                            ?>
                                            <option value='<?= $catItem->id ?>' <?= $selCat ?>><?= $catItem->title ?>
                                            </option>
                                            <?php endforeach;    ?>
                                        </select>
                                    </div>
                        ...
                        <table id="<?php echo $controller . "_" . $method ?>_DataTable"
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="icheck-primary">
                                            <input type="checkbox" value="<?= $row->id ?>" name="sel[]"
                                                id="post_<?= $row->id ?>">
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-sm mb-2"
                                            href="<?= route_to("edit_post", $row->id) ?>"><i
                                                class="fas fa-edit"></i></a>
                                        <?php if ($listtype !== 'deleted') : ?>
                                        <a class="btn btn-danger btn-sm mb-2 acpRmItem" title="Move to Trash"
                                            data-id="<?= $row->id ?>" data-delete="<?= route_to("delete_post") ?>"
                                            data-delete-message="Bạn có chắc chắn muốn xoá item này?"><i
                                                class="fas fa-trash"></i></a>
                                        <?php else : ?>
                                        <a class="btn btn-primary btn-sm mb-2" title="Recover Item"
                                            href="<?= route_to("recover_post", $row->id) ?>"><i
                                        ...
                                        <a class="btn btn-danger btn-sm mb-2 acpRmItem" title="Permanent Delete Item"
                                            ...
                                            data-delete="<?= route_to("permanent_delete_post", $row->id) ?>"
                                            data-id="<?= $row->id ?>">
