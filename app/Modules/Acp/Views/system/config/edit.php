        ...
        <form id="<?=$controller?>Form" role="form" method="post" class="form-horizontal" enctype="multipart/form-data" >
            ...
            <div class="card-body pad">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control" name="group_id">
                            <?php
                            foreach ($config->cfGroup as $key => $cfItem) :
                                $sel = ($key == $itemData->group_id) ? 'selected' : '';
                                ?>
                                <option <?=$sel?> value='<?=$key?>'><?=$cfItem['title']?></option>
                            <?php endforeach;    ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="title" class="form-control <?=session('errors.title') ? 'is-invalid' : ''?>" id="postInputTitle"
                               placeholder="<?=lang('Acp.cf_title')?>" value="<?=$itemData->title?>">
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="key" class="form-control <?=session('errors.key') ? 'is-invalid' : ''?>" id="postInputTitle"
                               placeholder="<?=lang('Acp.cf_key')?>" value="<?=$itemData->key?>">
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea class="form-control <?=session('errors.value') ? 'is-invalid' : ''?>" name="value" ><?=$itemData->value?></textarea>
                    </div>
                </div>
