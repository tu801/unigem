...
class ProductController extends AcpController
{
    ...
    public function addProductAction()
    {
        ...
        $rules = array_merge([
            'pd_name' => 'required|min_length[3]|is_unique[product_content.pd_name]|checkProductSlugExist',
        ], $this->ruleValidate());
    ...
    public function editProductAction($id)
    {
        ...
        try {
            ...
            // save meta data
            $this->_productContentModel->saveCustomMeta($id, $postData);
            $this->db->transCommit();
        } catch (DatabaseException $e) {
            $this->db->transRollback();
            return redirect()->back()->withInput()->with('errors', $this->_model->errors());
        ...
        $logData = [
            'title'        => 'edit Product',
            'description'  => "#{$this->user->username} đã sửa post #{$postData['pd_name']}",
