...
trait PostImage {
    ...
    /**
     * upload the post image and create thumb for the post
     * @param $postData
     * @param $image
     * @return \CodeIgniter\HTTP\RedirectResponse|string
     */
    public function uploadPostImage($postData, $image)
    {
        $info = [
            'file_name' => clean_url($postData['title']).'.'.$image->getClientExtension(),
            'sub_folder' => UploadFolderEnum::POST . '/' . date('Y/m')
        ];
    ...
    public function editPostImage(&$postData, $image, $post)
    {
        ...
        if ( $imgPath['success'] ) {
            ...
            create_thumb($imgThumb);
            delete_image($post->image, $info['sub_folder']);
