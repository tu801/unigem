const vConfigAttach = {
    props: [
        'imgData', 'imgDesc', 'selectImgType', 'renderType',
        'configData', 'oldData', 'inputName', 'returnImg',
    ],
    ...
    mounted() {
        if (this.renderType == 'config') {
            ...
            if (this.imgData) {
                ...
                let url = bkUrl + "/config/get-custom-attach?atts=" + this.imgData + '&dataType=' + dataType;
                $.ajax({
                    url: url,
                    type: 'GET',
                    dataType: 'json',
        ...
        setAttachFiles(images) {
            ...
            for (var i = 0; i < images.length; i++) {
                if (i == images.length - 1) {
                    ...
                    else if (this.returnImg == 'url') this.attImgs += images[i].full_image;
                } else {
                    if (this.returnImg == 'id') this.attImgs += images[i].id + ";";
                    else if (this.returnImg == 'url') this.attImgs += images[i].full_image + ";";
                }
