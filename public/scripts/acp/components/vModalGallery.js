...
const galleryImg = {
    ...
    methods: {
        showImageInfo(img){
            ...
            $('#'+imgclass).parent().find('img').removeClass('imgSelected');
            $('#'+imgclass).find('img').addClass('imgSelected');
            this.$emit('show-image-info', img);
        },
        deleteImg(url){
            let html =
                '<p>'+this.attItem.file_name+'</p>'
                +   '<img style="max-width: 100%;" src="'+base_url+'/'+this.attItem.thumb_image+'">';
...
const gallery = {
    ...
    methods: {
                        ...
                        if(this.allUploadData.length > 0) {
                            setTimeout(function() {
                                response.uploadData.forEach(resp => {
                                    myVue.allUploadData.push(Vue.util.extend({},resp));
                                });
                            }, 500);
                        } else {
        ...
        handleScroll (event) {
            var myVue = this;
            // console.log(myVue.scrollLoad);
            // console.log(event.target.scrollTop+event.target.clientHeight);
            if( event.target.scrollTop+event.target.clientHeight > myVue.scrollLoad-100
                &&  event.target.scrollTop < myVue.maxScrollLoad){
                if ( myVue.page > 0 && myVue.fetching == false) {
                    myVue.fetchImages();
                    myVue.fetching = true;
                }
            }
    ...
    mounted() {
        ...
        // const listElm = document.querySelector('#'+this.elm);
        // listElm.addEventListener('scroll', this.handleScroll, true);
